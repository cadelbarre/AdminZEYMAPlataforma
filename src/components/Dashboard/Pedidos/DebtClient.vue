    ...
    <div class="column">
      <div class="card">
        <div class="card-content" v-if="!loading">
          ...
          <b-tag
            v-if="clientSelected.dias !== null"
            type="is-info is-inline ml-1"
            >Credito a {{ clientSelected.dias }} días</b-tag
          >
        </div>
        <div class="card-content" v-if="loading">
    ...
    <div class="column">
      <div class="card">
        <div class="card-content" v-if="!loading">
          ...
          <b-tag
            v-if="infoDebtClient.available < 0"
            type="is-danger is-inline ml-1"
            >Sobrecupo</b-tag
          >
...
export default {
  ...
  methods: {
    ...
    getInfoDebtClient(x) {
      ...
      return dataDebt.reduce((acc, el) => acc + Number(el.valor), 0);
    },
    getDeposit(x) {
      let deposit = Object.values(x);
      return deposit.reduce(
        (acc, el) =>
          typeof el.abono == "string" ? Number(acc + el.abono) : acc,
        0
      );
    },
  },
